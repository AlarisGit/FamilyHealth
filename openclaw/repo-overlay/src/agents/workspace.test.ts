...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("omits HEARTBEAT.md when its content is effectively empty", async () => {
    const tempDir = await makeTempWorkspace("openclaw-workspace-");
  ...
  it("omits BOOTSTRAP.md from injected files after onboarding completion", async () => {
    const tempDir = await makeTempWorkspace("openclaw-workspace-");
    await ensureAgentWorkspace({ dir: tempDir, ensureBootstrapFiles: true });
    await writeWorkspaceFile({ dir: tempDir, name: DEFAULT_IDENTITY_FILENAME, content: "custom" });
    await writeWorkspaceFile({ dir: tempDir, name: DEFAULT_USER_FILENAME, content: "custom" });
    await fs.unlink(path.join(tempDir, DEFAULT_BOOTSTRAP_FILENAME));
    await ensureAgentWorkspace({ dir: tempDir, ensureBootstrapFiles: true });
    await fs.writeFile(path.join(tempDir, DEFAULT_BOOTSTRAP_FILENAME), "stale bootstrap", "utf-8");
  ...
  it("omits stale BOOTSTRAP.md for legacy completed workspaces with incomplete state", async () => {
    const tempDir = await makeTempWorkspace("openclaw-workspace-");
    await ensureAgentWorkspace({ dir: tempDir, ensureBootstrapFiles: true });
    await writeWorkspaceFile({ dir: tempDir, name: DEFAULT_IDENTITY_FILENAME, content: "custom" });
    await writeWorkspaceFile({ dir: tempDir, name: DEFAULT_USER_FILENAME, content: "custom" });
