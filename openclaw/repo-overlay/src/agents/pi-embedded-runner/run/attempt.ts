...
function serializeAgentMessageContent(content: unknown): string {
  ...
  return content
    .map((block) => {
      ...
      const type =
        typeof typedBlock.type === "string" && typedBlock.type.trim().length > 0
          ? typedBlock.type
          : "block";
      if (type === "image") {
        return "[image]";
      ...
      if (typeof typedBlock.text === "string") {
        return type === "text" ? typedBlock.text : `[${type}] ${typedBlock.text}`;
      }
      if (typedBlock.name !== undefined || typedBlock.arguments !== undefined) {
        return `[${type}] ${stringifyUnknown({
          name: typedBlock.name,
          arguments: typedBlock.arguments,
        })}`;
      }
      if (typedBlock.result !== undefined) {
        return `[${type}] ${stringifyUnknown(typedBlock.result)}`;
      }
      return `[${type}] ${stringifyUnknown(block)}`;
    })
    .join("\n");
}
...
function serializeAgentMessage(msg: AgentMessage): Record<string, unknown> {
  return {
    ...
    content: serializeAgentMessageContent((msg as { content?: unknown }).content),
  };
}

function resolveLlmTracePath(): string | null {
  const enabled = process.env.OPENCLAW_LLM_TRACE?.trim();
  const customPath = process.env.OPENCLAW_LLM_TRACE_PATH?.trim();
  if (!enabled && !customPath) {
    return null;
  }
  if (enabled && !["1", "true", "yes", "on"].includes(enabled.toLowerCase())) {
    return null;
  }
  return customPath || "/tmp/openclaw/llm-trace.jsonl";
}

function shouldTraceLlmRun(sessionKey?: string): boolean {
  const tracePath = resolveLlmTracePath();
  if (!tracePath) {
    return false;
  }
  const match = process.env.OPENCLAW_LLM_TRACE_SESSION_MATCH?.trim();
  if (!match) {
    return true;
  }
  return (sessionKey || "").includes(match);
}

async function writeLlmTraceRecord(record: Record<string, unknown>): Promise<void> {
  const tracePath = resolveLlmTracePath();
  if (!tracePath) {
    return;
  }
...
export async function runEmbeddedAttempt(
  ...
  try {
    ...
    try {
      ...
      let llmInputHistorySnapshot: Record<string, unknown>[] = [];
      let llmInputPrompt = "";
      let llmInputSystemPrompt: string | undefined;
      const onAbort = () => {
      ...
      try {
        ...
        llmInputHistorySnapshot = activeSession.messages.map(serializeAgentMessage);
        llmInputPrompt = effectivePrompt;
        llmInputSystemPrompt = systemPromptText;
      ...
      if (shouldTraceLlmRun(params.sessionKey ?? params.sessionId)) {
        try {
          await writeLlmTraceRecord({
            ...
            promptError: promptError ? describeUnknownError(promptError) : null,
            input: {
              systemPrompt: llmInputSystemPrompt,
              prompt: llmInputPrompt,
              historyMessages: llmInputHistorySnapshot,
            },
            output: {
              assistantTexts,
              lastAssistant:
                lastAssistant && typeof lastAssistant === "object"
                  ? {
                      role:
                        "role" in lastAssistant && typeof lastAssistant.role === "string"
                          ? lastAssistant.role
                          : "assistant",
                      content: serializeAgentMessageContent(
                        (lastAssistant as { content?: unknown }).content,
                      ),
                      provider:
                        "provider" in lastAssistant ? (lastAssistant as { provider?: unknown }).provider : undefined,
                      model: "model" in lastAssistant ? (lastAssistant as { model?: unknown }).model : undefined,
                    }
                  : null,
          ...
          log.warn(`llm trace write failed: ${String(traceErr)}`);
        }
      }
