...
async function shouldInjectBootstrapFile(resolvedDir: string): Promise<boolean> {
  ...
  const [identityPath, userPath] = [
    path.join(resolvedDir, DEFAULT_IDENTITY_FILENAME),
    path.join(resolvedDir, DEFAULT_USER_FILENAME),
  ];
  const [identityTemplate, userTemplate, identityContent, userContent] = await Promise.all([
    ...
    fs.readFile(identityPath, "utf-8").catch(() => ""),
    fs.readFile(userPath, "utf-8").catch(() => ""),
  ]);

  const hasUserContent = await (async () => {
    const indicators = [
      path.join(resolvedDir, "memory"),
      path.join(resolvedDir, DEFAULT_MEMORY_FILENAME),
...
export async function loadWorkspaceBootstrapFiles(dir: string): Promise<WorkspaceBootstrapFile[]> {
  ...
  const injectBootstrap = await shouldInjectBootstrapFile(resolvedDir);
  ...
  for (const entry of entries) {
    ...
    if (loaded.ok) {
      if (
        ...
        isHeartbeatContentEffectivelyEmpty(loaded.content)
      ) {
        continue;
      }
      result.push({
